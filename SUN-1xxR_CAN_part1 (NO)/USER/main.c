...
int main(void)
{
  ...
  printf("*   SunDUINO                                                 *\r\n");
    ...
    while (1)
	{
	  ...
	  delay_ms(1000);  /* opóznienie dla CAN 1000ms */
	  CanWriteData(0xA5A5);
	  
		/*LED informacyjny PD2 */
      if( DisplayLED ) 
	  { 
...
* Description    : Konfiguracja wektorów przerwan
*******************************************************************************/
void NVIC_Configuration(void)
{
  NVIC_InitTypeDef NVIC_InitStructure;

  /* Wlaczamy CAN1 RX0 w przerwaniach IRQ */
...
* Description    : KIonfiguracja pinów GPIO uzytych w programie
*******************************************************************************/
void GPIO_Configuration(void)
{
  ...
  /* Ustwienia CAN pin RX */
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
  GPIO_Init(GPIOB, &GPIO_InitStructure); 
  
  /* Ustawienia CAN pin TX */
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
  GPIO_Init(GPIOB, &GPIO_InitStructure);	   
 
  /**
  *	Informacyjna dioda LED -> PD2
  */					 
...
* Description    : Konfiguracja kontrolera CAN 
*******************************************************************************/
void CAN_Configuration(void)
{
  ...
  /* CANbps= Fpclk/((BRP+1)*((Tseg1+1)+(Tseg2+1)+1) CANbps=36000000/(45*(4+3+1))=100kHz */  //jesli Tseg1+1 = CAN_BS1_8tp
...
* Description    : Zapis danych do CAN
*******************************************************************************/
void CanWriteData(uint16_t ID)
{
  CanTxMsg TxMessage;
...
* Description    : Ustawia Niski priorytet dla handlera USB lub przerwan CAN RX0
*******************************************************************************/
